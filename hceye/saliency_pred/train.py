...
from dataloader import SaliconDataset, HighlightDataset, HighlightDataset_
from loss import *
import cv2
...
parser.add_argument('--cc_coeff',default=-3.0, type=float)
parser.add_argument('--sim_coeff',default=0.0, type=float)
parser.add_argument('--nss_coeff',default=0.0, type=float)
parser.add_argument('--nss_emlnet_coeff',default=0.0, type=float)
parser.add_argument('--nss_norm_coeff',default=0.0, type=float)
...
parser.add_argument('--model_saved_path',default="saved_models/cog_high.pt", type=str)
parser.add_argument('--model_pretrained_path',default="saved_models/salicon_pnas.pt", type=str)
...
base_dir = r"C:\Users\wuzek\OneDrive\Desktop\NewProject\new_dataset"

train_img_dir = os.path.join(base_dir, "images/train/")
train_gt_dir = os.path.join(base_dir, "labels/labels_high/train/")
train_fix_dir = os.path.join(base_dir, "fixs/fixs_high/train/")

val_img_dir = os.path.join(base_dir, "images/val/")
val_gt_dir = os.path.join(base_dir, "labels/labels_high/val/")
val_fix_dir = os.path.join(base_dir, "fixs/fixs_high/val/")
...
# Retrieve all image files from the directory, sorted to ensure pairing
train_img_files = sorted([f for f in os.listdir(train_img_dir) if not f.startswith('.')])
val_img_files = sorted([f for f in os.listdir(val_img_dir) if not f.startswith('.')])

train_dataset = SaliconDataset(train_img_dir, train_gt_dir, train_fix_dir, train_img_ids,highlight_mode='both')
val_dataset = SaliconDataset(val_img_dir, val_gt_dir, val_fix_dir, val_img_ids,highlight_mode='both')
