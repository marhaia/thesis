...
def kldiv(s_map, gt):
    eps = 1e-10  # Adjusted epsilon value

    # Ensure saliency map has non-negative values
    s_map = torch.clamp(s_map, min=0)

    # Normalizing s_map and gt to make them distributions
    s_map = s_map / (torch.sum(s_map, dim=(1, 2), keepdim=True) + eps)
    gt = gt / (torch.sum(gt, dim=(1, 2), keepdim=True) + eps)

    # Computing KLD
    kld = gt * torch.log((gt + eps) / (s_map + eps))

    # Return the mean KLD over the batch
    return torch.mean(torch.sum(kld, dim=(1, 2)))
...
def normalize_map(s_map):
    ...
    # Ensure max_s_map is greater than min_s_map by at least epsilon to avoid division by zero
    epsilon = 1e-8
    norm_s_map = (s_map - min_s_map) / (max_s_map - min_s_map + epsilon)

    # If the maximum and minimum are the same (flat image), the result will be a map of zeros.
    # You can choose to set it to zeros or handle it differently if necessary.
    norm_s_map[torch.isnan(norm_s_map)] = 0

    return norm_s_map

def similarity(s_map, gt):
    ''' Compute similarity between predicted and ground truth saliency maps '''
    if torch.any(torch.isnan(s_map)) or torch.any(torch.isnan(gt)):
        raise ValueError("NaN values detected in input saliency maps")
    ...
    # # Debug: Print the min and max values of the normalized maps
    # print("Normalized s_map min/max:", s_map.min().item(), s_map.max().item())
    # print("Normalized gt min/max:", gt.min().item(), gt.max().item())

    eps = 1e-8  # Small constant to prevent division by zero
    sum_s_map = torch.sum(s_map.view(batch_size, -1), dim=1, keepdim=True) + eps
    sum_gt = torch.sum(gt.view(batch_size, -1), dim=1, keepdim=True) + eps

    # Use torch.where to prevent division by zero
    expand_s_map = sum_s_map.unsqueeze(-1).expand_as(s_map)
    expand_gt = sum_gt.unsqueeze(-1).expand_as(gt)

    # Check for infinities after expanding
    if not torch.all(torch.isfinite(expand_s_map)):
        raise ValueError("expand_s_map contains non-finite values (inf or NaN)")

    if not torch.all(torch.isfinite(expand_gt)):
        raise ValueError("expand_gt contains non-finite values (inf or NaN)")

    # Perform the normalization
    s_map = s_map / expand_s_map
    gt = gt / expand_gt

    # Reshape to batch_size x -1 to compute similarity
    s_map = s_map.view(batch_size, -1)
    gt = gt.view(batch_size, -1)

    # Compute the similarity
    similarity_score = torch.mean(torch.sum(torch.min(s_map, gt), 1))

    # Debug: Check if the similarity score is finite
    if not torch.isfinite(similarity_score):
        raise ValueError("Non-finite similarity score detected")
...
def cc(s_map, gt):
    ...
    eps = 1e-8  # Small constant to prevent division by zero and sqrt of zero

    # Assuming aa and bb are the sums or products that could potentially be zero
    # Apply clamp or add a small epsilon to avoid division by zero or sqrt of zero
    aa = torch.clamp(aa, min=eps)
    bb = torch.clamp(bb, min=eps)

    return torch.mean(ab / (torch.sqrt(aa*bb)))


def nss(s_map, gt):
    eps = 1e-8  # A larger epsilon value for safety
    s_map = s_map.unsqueeze(1) if s_map.dim() == 3 else s_map
    gt = gt.unsqueeze(1) if gt.dim() == 3 else gt

    batch_size, _, s_map_height, s_map_width = s_map.size()
    gt_resized = F.interpolate(gt, size=(s_map_height, s_map_width), mode='bilinear', align_corners=False)

    # Flattening the maps for computation
    s_map_flat = s_map.view(batch_size, -1)
    gt_flat = gt_resized.view(batch_size, -1)

    # Only proceed with NSS computation if there are fixations
    if torch.sum(gt_flat) == 0:
        return torch.tensor(0.0).to(s_map.device)  # No fixations handling

    mean_s_map = torch.mean(s_map_flat, dim=1, keepdim=True)
    std_s_map = torch.std(s_map_flat, dim=1, keepdim=True)

    # Replace in-place operation with a non-in-place alternative
    std_s_map = torch.where(std_s_map < eps, torch.ones_like(std_s_map), std_s_map)

    s_map_norm = (s_map_flat - mean_s_map) / std_s_map
    s_map_norm = s_map_norm.view_as(s_map)  # Reshape to original dimensions

    s_map_norm = s_map_norm * gt_resized
    nss_score = torch.sum(s_map_norm.view(batch_size, -1), 1) / torch.clamp(torch.sum(gt_flat, 1), min=eps)

    return torch.mean(nss_score)
